...
pub async fn insert_cognito_user(response_body: &Body) {
    ...
    let username =
        user_attributes.username.to_string() + now_datetime.format("%H%M%S%f").to_string().as_str();
...
mod tests {
    ...
    #[tokio::test]
    async fn crud_users() {
        crate::insert_cognito_user(&Body::Empty).await;
        let result = 4;
        assert_eq!(result, 4);
    }
}
